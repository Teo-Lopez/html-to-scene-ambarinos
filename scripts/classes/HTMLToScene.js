...
class HTMLToScene {
  ...
  static get keepPlayerList() {
    return Boolean(this.flags.htmltoscene?.keepPlayerList ?? this.forcePlayerList);
  }

  static get spaceRight() {
    return Boolean(this.flags.htmltoscene?.spaceRight ?? this.respectRightControls);
  }

  static get rightDisabled() {
    return Boolean(this.flags.htmltoscene?.rightDisabled ?? this.disableRightControls);
  }

  static get hidePaused() {
    return Boolean(this.flags.htmltoscene?.hidePaused ?? this.disableGamePausedStatus);
  }

  static get hideSmallTime() {
    return Boolean(this.flags.htmltoscene?.hideSmallTime ?? this.disableSmallTime);
  }
  ...
  static get passData() {
    return Boolean(this.flags.htmltoscene?.passData ?? this.assistedBidirectionalAccess);
  }

  static get autoMacrosEnabled() {
    return Boolean(this.flags.htmltoscene?.autoMacrosEnabled ?? this.autoMacros);
  }

  /* Numbers */

  static get keepBottomControls() {
    return Number(this.flags.htmltoscene?.keepBottomControls ?? this.lowerUISettings);
  }

  static get dataUpdateRate() {
    return Number(this.flags.htmltoscene?.dataUpdateRate ?? this.allowedRateOfAccess);
  }

  static get iFrameRefreshRate() {
    return Number(this.flags.htmltoscene?.iFrameRefreshRate ?? this.iFrameRefreshingRate);
  }
  ...
  static get selfReadyMacroName() {
    return String(this.flags.htmltoscene?.selfReadyMacroName ?? this.htmltosceneReadyMacro);
  }

  static get iframeReadyMacroName() {
    return String(this.flags.htmltoscene?.iframeReadyMacroName ?? this.iframeReadyMacro);
  }

  static get iframeUpdatedMacroName() {
    return String(this.flags.htmltoscene?.iframeUpdatedMacroName ?? this.iframeUpdatedMacro);
  }
  ...
  static replace(...args) {
    ...
    if (!this._diceSoNiceInstalled) {
      document.body.insertBefore(this.createIframe(canvasHeight, canvasWidth), document.getElementById('pause'));
    } else {
  ...
  static calcSpacedWidth() {
    ...
    return (window.innerWidth || document.documentElement.clientWidth || document.body.clientWidth) - widthUImod;
  }
  ...
  static async getSceneHtml(settings) {
    return await renderTemplate('modules/html-to-scene/templates/sceneSettings.html', settings);
  }
  ...
  static swapPosition(nodeID) {
    ...
    if (otherNode != null && typeof otherNode == 'htmlelement') {
      ...
      getComputedStyle(this._iFrameNode).setProperty('z-index', otherZIndex - 1);
    }
  }
  ...
  static htmlToSceneReadyMacro() {
    if (this.enabled && this.autoMacrosEnabled) {
      (async () => {
        while (!window.hasOwnProperty('HTMLAccess')) await new Promise((resolve) => setTimeout(resolve, 10));
  ...
  static nodeVisibility(DOMNode, visibility) {
    if (DOMNode === '#logo' && visibility === 'visible' && !this.showFoundryLogo()) return; //showFoundryLogo setting
